    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (orderID != null) {
                ...
                List<OrderDetailDTO> result = dao.getDetailByID(orderid);
                OrderDTO order = dao.findOrderByID(orderid);
                request.setAttribute("DETAILS", result);
                request.setAttribute("ORDER", order);
                
            }
        } catch (NumberFormatException | SQLException | NamingException e) {
            log("Error at DetailHistoryController : " + e.getMessage());
        } finally {
            request.getRequestDispatcher("admin_detailHistory.jsp").forward(request, response);
        }
    }
